...
### 6.7 Capacitor (Ionic) — المخاطر

> **ما هو؟** Capacitor هو "الجيل التالي" من Cordova، يلف Web app في WebView Native ويُضيف JS-to-Native bridges. (Ionic Framework اختياري للـ UI).
> **مَن يستخدمه؟** Burger King، Sworkit، Untappd، MarketWatch.

| الخطر | الاحتمال | الأثر | التخفيف |
|-------|---------|------|---------|
| **يبقى WebView في القلب** — يرث معظم مشاكل الخيار F | حتمي | **عالٍ جداً** | لا تخفيف بنيوي |
| **أداء أقل من Native/RN** — خاصة في القوائم الطويلة (50,000 عميل) | عالٍ | عالٍ | Virtualization يدوي معقد |
| **Bluetooth Printer يتطلب plugin مخصص** — `@capacitor-community/bluetooth-le` لكن دعم ESC/POS غير قياسي | متوسط | عالٍ | كتابة plugin Bixolon JPOS من الصفر |
| **JS Bridge مُعرّض لنفس مشاكل `@JavascriptInterface` القديمة** — إذا فُتح للـ WebView محتوى خارجي | متوسط | **حرج** (نفس V18) | originWhitelist صارمة + CSP |
| **الاعتماد على Ionic** كـ UI Kit يخلق lock-in تجاري | متوسط | متوسط | استخدام Capacitor بدون Ionic ممكن لكن نخسر مكتبة المكونات |
| **Animations محدودة** — تعتمد على CSS، لا 60fps مضمون | عالٍ | متوسط | Lottie للحركات الثقيلة |
| **حجم APK كبير نسبياً** — ~20-30MB حتى مع تطبيق صغير | حتمي | منخفض | لا تخفيف |
| **مكتبة WatermelonDB غير مدعومة** — يستخدم SQLite plugin أبسط، لا Sync مدمج | حتمي | عالٍ | بناء طبقة Sync يدوياً (3-4 أسابيع) |
| **لا توافق مع `app1`** — مشروع مختلف بالكامل، لا إعادة استخدام للكود | حتمي | متوسط | لا تخفيف |
| **محرك WebView مختلف بين Android/iOS** — Chrome vs Safari، يتطلب اختبار مزدوج | حتمي | متوسط | E2E tests على كلتا المنصتين |

**خلاصة:** Capacitor يُعالج جزئياً مشكلة الـ Cross-Platform (يدعم iOS) لكنه **يُبقي على نفس النموذج الأساسي للتطبيق الحالي** (WebView + JS Bridge). لا يحل الجذور — يكسوها فقط بـ wrapper أحدث. **التوصية: مرفوض** للأسباب نفسها التي رفضنا بها WebView، مع ميزة وحيدة هي دعم iOS.

| المعيار | التقييم |
|---|---|
| **هل يحل ثغرات V1-V20؟** | ❌ يحل ~30% فقط (المتعلقة بالـ Bridge)، يُبقي على V6-V7 و V14 |
| **هل يدعم Offline-First بشكل أصلي؟** | ⚠️ جزئياً — IndexedDB يعمل لكن Sync يدوي |
| **هل يقدّم MVP في وقت أقصر؟** | ❌ لا — `app1/` يقدم Stack جاهز مع RN |
| **التقييم النهائي** | ⚠️ **3/10** — أفضل من WebView، أسوأ من RN/Flutter/Native |
